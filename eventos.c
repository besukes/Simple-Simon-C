...
void efetuaEventoClique(int matrizCartasJogo[10][21],undoMove * estadoUndoGlobal,SDL2Bases * args,SDL_Event event){
    ...
    //Clicou no botao de UndoMove
        ...
        desfazerJogada(matrizCartasJogo,estadoUndoGlobal);
    }
    //Clicou para reeniciar o jogo
    else if(dentroDoBotao(event,args,100,50,700,20)){
        reeniciaJogo(matrizCartasJogo,estadoUndoGlobal,args);
    }
...
void efetuaEventoSoltar(int matrizCartasJogo[10][21],undoMove * estadoUndoGlobal,SDL2Bases * args,SDL_Event event){
    ...
    if(eventoRelevante && cartaColocavel(matrizCartasJogo[linha][coluna],args->cartas[0])){
        colocaArrayCartas(matrizCartasJogo,estadoUndoGlobal,args);
        adicionaJogadaUndoMove(matrizCartasJogo,linha,args,estadoUndoGlobal);
    }
